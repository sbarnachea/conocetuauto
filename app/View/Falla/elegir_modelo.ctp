	...
	<div class="contenido falla">
		...
		<?php 
			echo $this->Form->create('Falla', array(
				'url' => array('controller' => 'falla', 'action' => 'elegir_modelo', $marca['Marca']['id']),
			...
			<table class="table-falla">
				...
				<tr>
					<td>Elige el modelo de <?php echo ucwords($marca['Marca']['nombre']); ?> </td>
					<td><?php echo $this->Form->input('Vehiculo.id_modelo', array('class' => 'form-control', 'options' => $modelos, 'required' => true, 'label' => false, 'empty' => 'Seleccionar Modelo')); ?></td>
				</tr>
				<tr>
					<td>Año Vehiculo</td>
			...
			<?php
			echo $this->Html->link('Cancelar',$this->Html->url(array('controller' => 'modelo', 'action' => 'index', $marca['Marca']['id'])), array('class' => 'pull-left cancelar'));
